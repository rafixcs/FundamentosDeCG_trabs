...
{
	// Check if its a odd number of sections
	// so it will divide into rows and columns
	/*if (n_sections % 2 == 0)
	{
		if (n_sections == 2)
			this->_sectionSize = width / 2.f;
		else
		{
	...
	this->_sectionSize = 100.f / n_sections;
	this->_width = width;
	this->_height = height;
	this->_block_sections.clear();
...
{
	return this->_block_sections[section];
}
